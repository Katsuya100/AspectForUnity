...
{
    public ref struct ProceedingContext<T>
    {
        ...
        {
            _state = state as IProceedingState<T> ?? throw new ArgumentException("Invalid ProceedingContext state.", nameof(state));
            _slot = slot;
        ...
        public ProceedingContext(object state, int slot, int generation, ref T initialReturnValue)
        {
            _state = state as IProceedingState<T> ?? throw new ArgumentException("Invalid ProceedingContext state.", nameof(state));
            _slot = slot;
            _generation = generation;
        ...
        public ref readonly T ReturnValue
        {
            get
            {
                if (_hasReturnReference)
